...
export default function ProjectReplacementDialog({action,onSave,onProceed,onCancel}:{action:string;onSave:()=>Promise<ProjectSaveOutcome>;onProceed:()=>Promise<void>;onCancel:()=>void}) {
 const dialog=useRef<HTMLDialogElement>(null),inFlight=useRef(false);
 const [busy,setBusy]=useState(false),[message,setMessage]=useState("");
 useEffect(()=>{const element=dialog.current!,previous=document.activeElement as HTMLElement|null;element.showModal();return()=>{element.close();previous?.isConnected&&previous.focus();};},[]);
 const proceed=async(save:boolean)=>{
  if(inFlight.current)return;
  inFlight.current=true;setBusy(true);setMessage("");
  try {
   if(save){const result=await onSave();if(result!=="saved"){
 ...
 return <dialog ref={dialog} className="project-replacement-dialog" aria-labelledby="project-replacement-title" aria-describedby="project-replacement-description" onCancel={event=>{event.preventDefault();if(!inFlight.current)onCancel();}} onKeyDown={event=>{
  ...
  <p id="project-replacement-description">This will replace the current project and clear its Undo history. Save your work before continuing?</p>
  {message&&<p role="status">{message}</p>}
  </div>
  <footer><button autoFocus disabled={busy} onClick={onCancel}>Cancel</button><button disabled={busy} onClick={()=>void proceed(false)}>Continue without saving</button><button className="primary" disabled={busy} onClick={()=>void proceed(true)}>{busy?"Please wait…":"Save and continue"}</button></footer>
 </dialog>;
}
