// Derived from Three.js 0.185.1 examples/jsm/loaders/FBXLoader.js (MIT).
// Copyright 2010-2026 Three.js authors. See FBXLoader.LICENSE.txt.
// Local change: preserve non-planar extruded polygon strips before planar Earcut.
// Local change: use worker-safe image URLs and camera aspect defaults.
// Keep upstream parser behavior otherwise; review these hooks when updating Three.js.
import { triangulateExtrudedPolygon } from '../extruded-polygon.ts';
...
class FBXTreeParser {
	...
	parseImage( videoNode ) {
		...
		if ( typeof content === 'string' ) { // ASCII format
			...
			return globalThis.URL.createObjectURL( new Blob( [ array ], { type: type } ) );
	...
	createCamera( relationships ) {
		...
		if ( cameraAttribute === undefined ) {
			...
			// A background import worker has no viewport. Explicit FBX aspect values
			// below still take precedence; cameras are filtered from the imported scene.
			let width = globalThis.innerWidth || 1;
			let height = globalThis.innerHeight || 1;
...
class GeometryParser {
	...
	genFace( buffers, geoInfo, facePositionIndexes, materialIndex, faceNormals, faceColors, faceUVs, faceWeights, faceWeightIndices, faceLength ) {
		...
		if ( faceLength > 3 ) {
			...
			triangles = triangulateExtrudedPolygon( vertices ) ?? ShapeUtils.triangulateShape( triangulationInput, [] );
	...
	addMorphTargets( parentGeo, parentGeoNode, morphTargets, preTransform ) {
		...
		// translation component of the geometric transform must not be applied to them â€” only the
